...
// ── Comm row ─────────────────────────────────────────────
function CommRow({ c, rowBg }) {
  const paises = arr(c.pais).slice(0, 3)
  const canal  = arr(c.canal)[0]
  const [d, m] = (c.date ?? '').split('-').reverse()
  return (
    <div className={cn('flex items-center gap-2.5 py-1.5 px-3 rounded-lg mx-2 cursor-default transition-colors', rowBg)}>
    ...
    <div className="mb-0.5">
      ...
      {open && (
        <div className="pb-2 space-y-0.5">
          {visible.map(c => <CommRow key={c.id} c={c} rowBg={group.rowBg} />)}
          {extra > 0 && (
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-2 min-h-0">
          {groups.length === 0 ? (
            ...
            groups.map(g => <NotifGroup key={g.id} group={g} />)
          )}
        </div>
